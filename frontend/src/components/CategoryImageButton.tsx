...
const CategoryImageButton: FC<CategoryProps> = ({categories, onClick}) => {
    return (
        <div className='w-auto h-8 flex justify-start cursor-pointer'>
            {categories?.map((arg: CategoryButtonType, index:number) => {
                return (
                    <div
                        key={index}
                        onClick={() => onClick}
                        className='h-8 rounded-[16px] flex justify-between m-0 bg-amber-20　border-black border border-solid-1'
                    >
